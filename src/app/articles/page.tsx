...
const page = () => {
  const FeaturedArticle = ({ para, title, image, link, readtime }: any) => {
    return (
      <>
        <article className="p-12 col-span-6 flex flex-col items-center justify-between rounded-3xl border border-solid border-dark bg-light shadow-solid">
          <Link
          ...
          <div className="w-full  flex flex-col items-start justify-between ">
            <Link
              ...
              href={link}
            >
              <h3 className="capitalize my-2 w-full text-left text-3xl font-bold">
                {title}
              </h3>{" "}
            </Link>
            <p className="text-dark/80 text-base">{para}</p>
            <span className="text-primary font-medium pt-4 text-lg">
              {readtime}&nbsp;min read
            </span>
          </div>
        </article>
  ...
  function handleMouseLeave(event:any){
    ...
    return (
      <>
        ...
        viewport={{once:true}}
        className="group border shadow-medium border-dark rounded-xl  w-full flex flex-row items-center justify-between h-auto py-6 px-4">
          <Link
            onMouseMove={handleMouse}
            onMouseLeave={handleMouseLeave}
            href={link}
            className="relative  w-[80%] hover:underline  capitalize text-xl font-bold"
          >
            {title}
          <FramerImage initial={{opacity:0}} whileInView={{opacity:1,transition:{duration:0.5}}} style={{x:x,y:y}} className="rounded-xl border border-black absolute hidden z-10" ref={imgRef} alt={title} src={img} height={400} width={400} ></FramerImage>
          
          </Link>
          <span className="text-primary text-lg font-medium">{date}</span>
        </motion.article>
      </>
    );
  };
  ...
  return (
    <main className="mb-24 flex mx-32 flex-col items-center justify-between">
      <AnimatedText
        className="text-8xl pt-16 pb-16 "
      ...
      <div className="w-full flex flex-col pb-32 justify-center items-center pt-32 ">
        <h1 className="text-4xl font-bold text-dark pb-16">All Articles</h1>
